...
@section("title","Secmo Doctor Transaction History")
@push('css')
...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Secmo Doctor Transaction History</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">Home</a></li>
                        <li class="breadcrumb-item active">Secmo Doctor Transaction History</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
            ...
            <div class="col-12">
                <div class="card card-info card-outline">
                    <div class="card-header">
                        <h3 class="card-title float-left">Secmo Doctor Transaction History</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body table-responsive">
                        <table class="table table-striped">
                            ...
                            <tbody>
                            @foreach($doctor_transactions as $key => $doctor)
                                <tr>
                                    <td>{{$doctor->trx}}</td>
                                    <td>{{$doctor->trx_type}}{{$doctor->amount}}</td>
                                    <td>{{$doctor->post_balance}}</td>
                                    <td>{{$doctor->details}}</td>
                                    <td>{{ date_format($doctor->created_at, 'jS F Y')}} </td>
