...
@section("title","Patient Transaction History")
@push('css')
...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Patient Transaction History</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">Home</a></li>
                        <li class="breadcrumb-item active">Patient Transaction History</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
            ...
            <div class="col-12">
                <div class="card card-info card-outline">
                    <div class="card-header">
                        <h3 class="card-title float-left">Patient Transaction History</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body table-responsive">
                        <table  class="table table-striped">
                            ...
                            @foreach($patientLists as $key => $patientList)
                                <tr>
                                    <td>{{$patientList->trx}}</td>
                                    <td>{{$patientList->trx_type}}{{$patientList->amount}}</td>
                                    <td>{{$patientList->post_balance}}</td>
                                    <td>{{$patientList->details}}</td>
                                    <td>{{ date_format($patientList->created_at, 'jS F Y')}} </td>
